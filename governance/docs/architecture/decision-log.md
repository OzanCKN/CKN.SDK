...
- **Sonuçlar:** Core projesi %100 bağımlılıksız (zero-dependency) kalmaya devam etti. İsteyen projeler ASP.NET Core kullanmadan da (örneğin Worker servisleri) CKN.SDK'yı rahatça kullanabilir. Web tabanlı API'ler ise `CKN.Sdk.AspNetCore` modülünü projesine dahil ederek standart hata yönetimini elde eder.
